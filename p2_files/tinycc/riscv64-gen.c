...
static void EB(uint32_t opcode, uint32_t func3,
               uint32_t rs1, uint32_t rs2, uint32_t offset)
{
...
ST_FUNC void gfunc_prolog(Sym *func_sym)
{
    ...
    /*
     * TODO: The following line reserves room for up to 5 4-byte
     * instructions in the prologue, to be filled in later (and
     * nop-padded if not all 5 are needed).  If you want to add
     * instructions to the prologue, you will need to edit this.
     */
    ind += 7 * 4;
    // We added this
    // ES(0x23,3,1,31,0);
...
ST_FUNC void gfunc_epilog(void)
{
    ...
    /*
     * TODO: here is where the function epilogue is filled in.
     * You can change this code to add more instructions to the
     * function epilogue to check the return address against the
     * copy on the shadow stack and to crash the program if they
     * don't match.
     */
    ...
    if (v >= (1 << 11)) {
        ...
        /*
         * TODO: this jumps back into the function after the prologue; if
         * you change the size of the prologue you should change this
         * offset, too.
         * (The instructions here are logically part of the prologue to
         * and used to handle an awkward case with immediate encoding,
         * discussed below.)
         */
        gjmp_addr(func_sub_sp_offset + 7*4);
    }
    saved_ind = ind;

    /*
     * TODO: here is where the function prologue is actually filled
     * in.
     * You can change this code to add more instructions to the
     * function prologue to save the return address to the shadow
     * stack.
     * The current instructions reserve d bytes for the activation
     * record, save the old ra and s0 on the stack, and set s0 to be
     * a frame pointer.  Depending on how big d is, this may or may
     * not fit in a 12-bit immediate field; if it doesn't, that's
     * handled with a jump to some extra code after the ret, which
     * was set up earlier in this routine.
     */
    ...
    /*
     * TODO: TinyCC reserved room for 5 4-byte instructions in the
     * function prologue, but might have needed only 4 of them; in
     * that case, it pads with a nop.
     * If you add more instructions to the prologue, you'll need to
     * change the calculation here.
     */
    if ((ind - func_sub_sp_offset) != 7*4)
